...
You can drop this in the `render` method. This checks the current `isFave` `state` for true or false.

If it's true, it sets the `const` variable `isFave` to `remove_from_queue`; when it's false, it sets the `const` variable `isFave` to `add_to_queue`.
...
This line in particular checks if the `filter` `state` is currently "all"; if it is, it sets the value to `is-active`. If it isn't, it does nothing. `{this.state.filter === 'all' ? 'is-active' : ''}`
</details>
