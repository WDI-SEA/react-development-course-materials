...
- Imagine that you are a user of [Trello](https://trello.com/), a popular project management app.

- This app is great for managing and keeping track of grocery lists, chores, personal projects, and more. As we see above, this can be used to add and remove information dynamically — all within the browser. How have we handled dynamic data so far in React? How could we manage to change this data?
...
- Before getting started, let's make sure our `hello_world` project is up and running.

- Be sure to switch which app is running locally!

- In React, we are able to handle data in one of two ways:

- `props` **represent** data that is **immutatable** or read-only. Let's see what happens when you try to directly change `this.props.name` in the `Hello` component.
...
- Use the existing React repo to show what happens if you directly manipulate props. Make sure the students do not code along and instead are focused on seeing what happens on the instructor's machine. Your error message should looks something like this.
...
- Finally, we will add `this.state = {}`. This sets the initial values for our data that is changeable.
...
- Much cleaner, right? Throughout the course we're going to use this newer way to define `state`, but you may run into the `constructor` syntax when doing your own research.
...
- If students do not have much experience with events, make sure to talk through the `event` keyword. A good way to explore the `event` keyword is to explore the object when adding this event listener to the DOM.
...
Check out the React documentation on supported events: https://reactjs.org/docs/events.html#supported-events
...
> CFU: If we were just working with regular old JavaScript, what could we use to increase the number of a variable by 1?
...
> If you're interested in reading more in depth about this, here is more on what [should and shouldn't go in `state`](https://facebook.github.io/react/docs/state-and-lifecycle.html). This link is also in the Further Reading page at the end of the React module.
