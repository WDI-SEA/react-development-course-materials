...
- You may need to restart your terminal session before moving forward. Why? Because you've just installed software to modify your terminal's environment, and your environment variables may not be updated yet.
...
- Log in to Heroku using the command line. Do so by typing in `heroku login`. You'll be prompted to enter your Heroku credentials followed by an authentication message as seen here.
...
- Now, we need to initialize a Git repository inside our React application. The Heroku CLI requires Git to handle versioning for deployment.
...
- A buildpack is a set of scripts that Heroku will use to read your React.js code and rebuild it for hosting on the web. You shouldn't skip this step.
...
- You'll see Heroku output something similar to what's seen here.
...
- In theory, we'll have actually built an application. For our purposes, we're going to use what has been provided with `create-react-app`. We'll use Git to add all of the files and commit our changes.
...
- Finally, we can push our app to Heroku. By doing this, Heroku will put your app together on the internet.

- After seeing lots of command line wizardry happen before your eyes, you should see Heroku print out a URL to the console. Copy and paste that URL into your browser, or type `heroku open`. Either way, you'll be taken to your React application.
...
- Maybe you want to embed an image of a cat that you found on `https://acatwebsite.com/` (this is just a sample URL). However, maybe the owners of that website want to make sure that their cat images can **only** be accessed through a browser, and not, for example, a JavaScript POST call.

- Limiting who can access your web server is important in web security. Otherwise, someone could write code that runs on `https://acatwebsite.com/` and changes all the content to be images of dogs.
...
- With a CORS proxy, you don't have to know anything about setting up CORS (unless you're interested in researching it yourself); it takes care of this for you. You just need the proxy, and then you can embed all the cat pictures you want.
